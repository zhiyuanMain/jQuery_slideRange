...
(function($, window, document, undefined) {
	...
	slideRange.prototype = {
		...
		render: function() {
			...
			if (this.inputNode.width() === 0 && !this.options.width) {
				console.log('slideRange : no width found, returning');
				return;
			} else {
	...
	/*$.slideRange = function (node, options) {
		var jNode = $(node);
		if(!jNode.data('slideRange')){
			jNode.data('slideRange', new slideRange(node, options));
		}
		return jNode.data('slideRange');
	};

	$.fn.slideRange = function (options) {
		return this.each(function(){
			$.slideRange(this, options);
		});
	};*/

	var pluginName = 'slideRange';
	...
	$.fn[pluginName] = function(option) {
		...
		this.each(function() {
			...
			if (!data) {
				$this.data('plugin_' + pluginName, (data = new slideRange(this, options)));
